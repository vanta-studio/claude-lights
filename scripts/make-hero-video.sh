...
ACT1_START=0.6   # calm: sessions working, panel browse
ACT1_END=9.8
ACT2_START=9.8   # icon flips red, badge, panel shows "Needs input"
ACT2_END=17.0
ACT3_START=19.0  # resumed: back to yellow, then all done
ACT3_END=25.7
...
ffmpeg -y -v error \
  ...
  -filter_complex "
    ...
    [act2base][ov2]overlay=0:0:eof_action=repeat:enable='between(t,0.5,6.5)',format=yuv420p[act2];
    ...
    [title][act1][act2][act3][end]concat=n=5:v=1:a=0,format=yuv420p[out]
  " \
...
# README hero GIF: the zoomed red-alert + resolution acts (no cards),
# palette-optimized, kept well under GitHub's comfortable size range.
GIF="$ROOT/docs/media/hero.gif"
ffmpeg -y -v error -ss 12.0 -to 26.0 -i "$OUT" \
